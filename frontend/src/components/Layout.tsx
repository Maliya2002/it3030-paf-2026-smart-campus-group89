    ...
    <div className="flex h-screen bg-dark-900 overflow-hidden">
      ...
      `}>
        ...
        <div className="flex items-center gap-3 p-6 border-b border-dark-700">
          <div className="w-10 h-10 bg-primary-500 rounded-xl flex items-center justify-center flex-shrink-0 shadow-glow">
            <span className="text-white font-black text-sm">SC</span>
          </div>
          <div>
            <h1 className="text-white font-bold text-lg leading-none">Smart Campus</h1>
            <p className="text-dark-500 text-xs">Operations Hub</p>
          </div>
          <button
            className="ml-auto lg:hidden text-dark-400"
            onClick={() => setSidebarOpen(false)}
          >
            <X size={20} />
          </button>
        ...
        <nav className="flex-1 p-4 overflow-y-auto">
          <div className="space-y-1">
            {visibleNavItems.map(item => (
              ...
              >
                ...
                {item.adminOnly && (
                  <Shield size={12} className="text-red-400 opacity-70" />
                )}
              </NavLink>
            ))}
          </div>
          ...
          <div className="mt-6 p-3 bg-primary-500/10 border border-primary-500/20 rounded-xl">
            <p className="text-primary-400 text-xs font-semibold">SLIIT · FoC</p>
            <p className="text-dark-500 text-xs">IT3030 PAF 2026</p>
          </div>
        </nav>
        ...
        <div className="p-4 border-t border-dark-700">
          <div className="flex items-center gap-3 p-3 bg-dark-700/50 rounded-xl mb-3">
            {user?.profilePicture ? (
              <img
                ...
                alt={user.name}
                className="w-9 h-9 rounded-lg border border-primary-500/30"
              />
            ) : (
          ...
          <div className="grid grid-cols-2 gap-2">
            <button
              ...
              className="flex items-center justify-center gap-2 px-3 py-2 bg-dark-700 hover:bg-dark-600 rounded-lg text-dark-300 hover:text-white text-sm transition-all"
            >
              <User size={14} />
              Profile
            </button>
            <button
              ...
              className="flex items-center justify-center gap-2 px-3 py-2 bg-red-500/10 hover:bg-red-500/20 rounded-lg text-red-400 hover:text-red-300 text-sm transition-all"
            >
              <LogOut size={14} />
              Logout
            </button>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="bg-dark-800 border-b border-dark-700 px-6 py-3 flex items-center gap-4">
          <button
            className="lg:hidden p-2 bg-dark-700 rounded-lg text-dark-300"
          ...
          >
            {user?.profilePicture ? (
              <img
                src={user.profilePicture}
                alt={user.name}
                className="w-8 h-8 rounded-lg border border-primary-500/30"
              />
            ) : (
